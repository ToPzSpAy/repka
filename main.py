...
class AuthWindow(QWidget):
    ...
    def signin(self):
        img = ''
        if self.path:
            files = os.listdir(path="avatars")
            _, ext = os.path.splitext(self.path)
            img = 'img' + str(len(files)) + ext
            shutil.copy(self.path, 'avatars/'+img)
...
class Appw(QWidget):
    ...
    def user_info(self,item):
        ...
        if self.usr[3] >= 1:
            ...
            self.pix = QPixmap('avatars/' + self.cuser[8])
    ...
    def edit_ur_data(self):
        if self.usr[3] >= 2:
            SqlDB().edit(self, self.cuser[0])
        else:
            SqlDB().edit(self, self.usr[0])
        tusr = SqlDB().user_data(self.usr[1])
        if self.usr != tusr:
            self.usr = tusr
        self.user_list()
    ...
    def delacc(self):
        ...
        if ret == QMessageBox.Yes:
            if self.usr[3] >= 2:
                SqlDB().delete(self.cuser)
                self.user_list()
            else:
                SqlDB().delete(self.usr)
                self.w = AuthWindow()
                self.w.show()
                self.close()
